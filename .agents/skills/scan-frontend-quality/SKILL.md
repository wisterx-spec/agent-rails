...
```bash
# 示例：检测物理色
grep -rn "bg-blue-\|bg-red-\|bg-green-\|bg-gray-\|text-blue-" {{FRONTEND_PATH}} --include="*.tsx"

# 示例：检测裸 hex
grep -rn "#[0-9a-fA-F]\{3,6\}" {{FRONTEND_PATH}} --include="*.tsx" --include="*.css"

# 示例：检测 alert
grep -rn "window\.confirm\|window\.alert\|^\s*alert(" {{FRONTEND_PATH}} --include="*.tsx"
```
