...
读取并执行 `.agents/workflows/_phases/proposal-gate.md` 中的完整步骤（跳过 auto-dev 技术补充部分）。
...
### 5.5. 📋 规格文件交接 (Spec Handoff - Conditional)
**条件触发**：当改动预计超过 3 个文件、包含多个可拆 Task、需要独立执行器处理，或用户明确要求 planner/executor 分工时执行。

Planner 负责写清楚意图和验收标准，Executor 负责实现和验证。交接物是 spec 文件，不是口头描述。

**写入位置**：读取 `project.config.json` 的 `agent.signal_dir`，缺省为 `tmp/agent-signals`。

**文件命名**：`spec-{kebab-case-feature}-{YYYYMMDD_HHMMSS}.md`
...
**Anti-hallucination check before dispatch**:

- Every named file exists or the spec clearly marks it as new.
- Every referenced function, class, route, field, or command has been verified with search or file reads.
- Every acceptance item can be verified by command, file check, API call, or is explicitly marked manual.
- Test commands avoid external paid/production services unless the spec states the mock boundary.

After the spec is written and reviewed, call `.agents/skills/run-pending-specs/SKILL.md` or hand the spec to a human-approved executor. Any executor must write a matching `session-{slug}-*.md` file before the handoff is considered complete.
...
**跳过条件**：本次没有 ORM / migration / schema 变更。
**目的**：为每一个字段、每一个新索引自动对比线上表生成增量 DDL 脚本片段 `ALTER TABLE...`，并带上真实 Query SQL 业务逻辑供 DBA Review。

### 8. 🛡️ 测试阻断网 (Test Gates)
一切功能写完准备提交前，调用 `run-tests --mode=fast` 扫描。`run-tests` 优先执行 `project.config.json → testing.commands.fast`，缺失时才按项目上下文 fallback。
**红线约束**：如果有抛错，打回修改代码直至全 Pass 为止，决不允许带 Bug 的逃逸提交。

### 9. 📦 包装及移交 CI/CD (Smart Commit Handoff)

读取并执行 `.agents/workflows/_phases/commit-handoff.md` 中的 Step 1-4（跳过 auto-dev 专有步骤）。
